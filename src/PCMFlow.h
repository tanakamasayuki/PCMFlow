...
public:
    ...
    //
    // Use these instead of hand-rolling `frames * channels * bytesPerSample`
    // at call sites. The runtime form picks up the active output format;
    // the constexpr `maxBytesForFrames()` gives a worst-case bound usable
    // for compile-time array sizing (stereo 16-bit = 4 bytes/frame).

    size_t bytesPerFrame() const { return outFormat_.bytesPerFrame(); }
    size_t bytesForFrames(size_t frames) const { return frames * bytesPerFrame(); }

    static constexpr size_t maxBytesPerFrame() { return 2u * 2u; } // 2ch * 16-bit
    static constexpr size_t maxBytesForFrames(size_t frames)
    {
        return frames * maxBytesPerFrame();
    }

    // Type-safe overload: deduces the buffer's byte capacity from the
    // declared array size and clamps `frameCount` so the call can never
    // overrun. Returns the number of frames actually copied.
    //
    // Recommended pattern — byte array sized via the helpers:
    //
    //   // Compile-time worst case (stereo 16-bit = 4 bytes/frame):
    //   static uint8_t buf[PCMFlow::maxBytesForFrames(256)];
    //   size_t got = audio.readFrames(buf, 256);
    //
    //   // Runtime exact size (GCC VLA — Arduino default toolchain supports it):
    //   uint8_t buf[audio.bytesForFrames(256)];
    //   size_t got = audio.readFrames(buf, 256);
    //
    // The element type doesn't matter — the template clamps frameCount so
    // that `frameCount * bytesPerFrame() <= sizeof(buf)`. A too-small buffer
    // yields a short read instead of memory corruption.
    template <typename T, size_t N>
    size_t readFrames(T (&out)[N], size_t frameCount)
    {
        const size_t bpf = bytesPerFrame();
        if (bpf == 0)
            return 0;
        const size_t cap = (sizeof(T) * N) / bpf;
        const size_t safe = (frameCount < cap) ? frameCount : cap;
        return readFrames(static_cast<void *>(out), safe);
    }
