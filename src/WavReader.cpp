...
bool WavReader::begin(ByteStream* input) {
    ...
    uint8_t hdr[12];
    if (!readFully(hdr, sizeof(hdr))) { error_ = Error::ReadError; return false; }
    if (!fourcc_eq(hdr + 0, "RIFF")) { error_ = Error::NotRiff; return false; }
    if (!fourcc_eq(hdr + 8, "WAVE")) { error_ = Error::NotWave; return false; }
    ...
    while (true) {
        ...
        if (fourcc_eq(chunkHdr, "data")) {
            if (!fmtSeen) { error_ = Error::NoFmtChunk; return false; }
...
size_t WavReader::readFrames(void* out, size_t frameCount) {
    ...
    if (framesRemaining_ == 0) return 0;
